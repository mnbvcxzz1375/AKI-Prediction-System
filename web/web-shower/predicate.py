...
# --------------------------
class AKIPredictor:
    def __init__(self, model_dir="./public/models"):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"当前设备：{self.device}")
        self.lab_features = ['aniongap_min', 'aniongap_max', 'albumin_min', 'albumin_max', 'bands_min', 'bands_max', 'bicarbonate_min', 'bicarbonate_max', 'bilirubin_min', 'bilirubin_max', 'creatinine_min', 'creatinine_max', 'chloride_min', 'chloride_max', 'glucose_min', 'glucose_max', 'hematocrit_min', 'hematocrit_max', 'hemoglobin_min', 'hemoglobin_max', 'lactate_min', 'lactate_max', 'platelet_min', 'platelet_max', 'potassium_min', 'potassium_max', 'ptt_min', 'ptt_max', 'inr_min', 'inr_max', 'pt_min', 'pt_max', 'sodium_min', 'sodium_max', 'bun_min', 'bun_max', 'wbc_min', 'wbc_max', 'gender']
        self.micro_features = ['spec_itemid', 'org_itemid', 'isolate_num', 'ab_itemid', 'dilution_text', 'dilution_value', 'urineoutput', 'gender']
        self.micro_imputer = joblib.load(f"{model_dir}/microbiologyevents_plus_lgbm.pkl")
        # print(self.micro_imputer)
        self.lab_imputer = joblib.load(f"{model_dir}/labs_first_day_lgbm.pkl")
        # print(self.lab_imputer)
        self.micro_scaler = joblib.load(f"{model_dir}/microbiologyevents_plus_standard_scaler.pkl")
        # print(self.micro_scaler)
        self.lab_scaler = joblib.load(f"{model_dir}/labs_first_day_lgbm_standard_scaler.pkl")
        # print(self.lab_scaler)
...
def generate_charts(micro_df, lab_df):
    ...
    if 'feature1' in micro_df.columns and 'feature2' in micro_df.columns:
        micro_scatter = AKIVisualizer.plot_micro_scatter(micro_df, 'feature1', 'feature2')
        micro_scatter.write_html(os.path.join(output_dir, "micro_scatter.html"))
        micro_density = AKIVisualizer.plot_micro_density_contour(micro_df, 'feature1', 'feature2')
        micro_density.write_html(os.path.join(output_dir, "micro_density.html"))

    lab_corr = AKIVisualizer.plot_lab_correlation_heatmap(lab_df)
    lab_pair = AKIVisualizer.plot_lab_pairplot(lab_df)
    lab_violin = AKIVisualizer.plot_lab_violin(lab_df)
    lab_parallel = AKIVisualizer.plot_parallel_coordinates(lab_df)
    radar = AKIVisualizer.plot_radar_chart(probs)
    sunburst = AKIVisualizer.plot_sunburst_chart(lab_df)
